...
def rewrite_query(article: Article, llm_client: LLMClient) -> str:
    ...
    """
    prompt = (
        f"Given this Wikipedia article title and the fact that no relevant news "
        f"was found in the past week, generate a 4-6 word search query that might "
        f"explain why it is suddenly getting traffic. Output ONLY the query, nothing else.\n\n"
